...
# Por que é ligação: origem e destino em domínios diferentes (faturamento e barramento): donos distintos pedem permissão dos dois lados
# Canal declarado no bloco: evento
#
# Ligação tem permissão dos DOIS lados e state próprio. Ela mora no live de
# quem tem a permissão de criar, que aqui é o domínio barramento.
