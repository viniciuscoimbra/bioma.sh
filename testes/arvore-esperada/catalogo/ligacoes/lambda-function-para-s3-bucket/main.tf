...
# Por que é ligação: origem e destino em domínios diferentes (plataforma e plataforma/dados): donos distintos pedem permissão dos dois lados
# Canal declarado no bloco: direto
#
# Ligação tem permissão dos DOIS lados e state próprio. Ela mora no live de
# quem tem a permissão de criar, que aqui é o domínio plataforma/dados.
